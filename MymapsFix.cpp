...
void connectLocations(string fileName) {
  ...
  if (!isConnected) {
    ...
    // menambahkan data koneksi ke dalam txt
...
void loadData(const string &fileName) {
  ...
  if (inFile.is_open()) {
    ...
    bool isConnectionSection = false;
...
void deleteLocation(string fileName) {
  ...
  if (outFile.is_open()) {
    ...
    outFile << "Koneksi" << endl;
    for (int i = 0; i < numLocations; i++) {
      for (int j = 0; j < locations[i].numConnections; j++) {
        outFile << i << " " << locations[i].connections[j] << " "
                << locations[i].weights[j] << endl;
      }
    }
